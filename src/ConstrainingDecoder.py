...
class ConstrainingDecoder(BaseModel):
    ...
    def constrain_function(self, logits: list[float]) -> FunctionDefinition:
        """Mask the logits so that only single-digit function indexes
        remain, then return the function whose index scores highest."""
        available_tokens_id = {
    ...
    def constrain_parameter(
        ...
        """Dispatch to the constrained generator matching *param_type*."""
    ...
    def generate_string(
        ...
        """Generate freely until the model has produced a full quoted
        value, then extract it and ground it back onto the prompt."""
    ...
    def generate_number(
        ...
        """Only allow tokens made of numeric characters, then parse the
        accumulated text and ground it back onto the prompt."""
    ...
    def generate_boolean(
        ...
        """Compare the best "true" token score against the best "false"
        token score and keep the winner."""
        true_tokens = [
