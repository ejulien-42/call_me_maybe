from src.ConstrainingDecoder import ConstrainingDecoder
from src.Parser import FunctionDefinition, PromptEntry
...
class GenerationPipeline(BaseModel):
    ...
    _function: Optional[FunctionDefinition] = PrivateAttr(default=None)
    ...
    def generate_parameters(self, decoder: ConstrainingDecoder) -> None:
        ...
        items = list(self._function.parameters.items())
        for i, (param_name, param) in enumerate(items):
            tokens += self.llm_encode(f'"{param_name}": ')
            value, tokens = decoder.constrain_parameter(
                tokens=tokens,
                param_type=param.type,
                eos_token_id=eos_token_id,
            )
            self._output['parameters'][param_name] = value
            if i < len(items) - 1:
    ...
    def get_parameters_system_prompt(self) -> str:
        ...
        params_str = "\n".join(
            f"{name}: {param.type}"
            for name, param in self._function.parameters.items()
        )
    ...
    def llm_encode(self, text: str) -> Any:
        """Encode *text* and return a flat list of token ids."""
